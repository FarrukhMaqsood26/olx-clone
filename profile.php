...
<main class="flex-grow py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full bg-red">
    ...
    <div
        ...
        <div class="flex-1 text-center md:text-left">
            ...
            <div class="flex flex-col md:flex-row gap-2 md:gap-6 text-sm text-slate-600 mb-6 font-medium">
                <span class="flex items-center justify-center md:justify-start gap-2"><i
                        class="far fa-envelope text-slate-400"></i>
                    <?= htmlspecialchars($user['email']) ?>
                </span>
                <?php if ($user['phone']): ?>
                    <span class="flex items-center justify-center md:justify-start gap-2"><i
                            class="fas fa-phone text-slate-400"></i>
        ...
        <div class="mt-4 md:mt-0 flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <button onclick="toggleEditForm()"
                class="w-full sm:w-auto bg-white border-2 border-slate-200 hover:border-brand text-slate-700 hover:text-brand font-bold py-2.5 px-6 rounded-full transition shadow-sm flex items-center justify-center gap-2">
                <i class="fas fa-pen"></i> Edit Profile
            </button>
            <a href="api/auth.php?action=logout" onclick="return confirm('Are you sure you want to log out?')"
                class="w-full sm:w-auto bg-red-50 border-2 border-red-200 hover:bg-red-500 hover:border-red-500 hover:text-white text-red-600 font-bold py-2.5 px-6 rounded-full transition shadow-sm flex items-center justify-center gap-2">
    ...
    <div id="editProfileForm"
        ...
        <form action="api/auth.php?action=update_profile" method="POST" class="space-y-6" enctype="multipart/form-data">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">New Password (leave blank to
                        keep)</label>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button type="button" onclick="toggleEditForm()"
                    class="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-bold py-2.5 px-6 rounded-lg transition">Cancel</button>
                <button type="submit"
                    class="bg-brand hover:bg-brand-light text-white font-bold py-2.5 px-8 rounded-lg shadow-sm transition">Save
                    Changes</button>
            </div>
        </form>
    </div>
    ...
    <div class="mb-6 flex overflow-x-auto no-scrollbar gap-2 border-b border-slate-200">
        <button class="profile-tab active whitespace-nowrap pb-3 px-4 font-bold text-brand border-b-2 border-brand"
            onclick="filterAds('all', this)">All Ads (
            <?= $totalAds ?>)
        </button>
        <button
        ...
        <button
            class="profile-tab whitespace-nowrap pb-3 px-4 font-semibold text-slate-500 border-b-2 border-transparent hover:text-slate-800"
            onclick="filterAds('sold', this)">Sold (
            <?= $soldAds ?>)
        </button>
    </div>
    ...
    <div id="adsContainer" class="space-y-4">
                    ...
                    <div class="flex-1 flex flex-col justify-between">
                        <div>
                            <div class="flex items-start justify-between gap-4 mb-2">
                                ...
                                ?>
                                <span
                                    class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest rounded-full <?= $bg ?>">
                                    <?= ucfirst($ad['status']) ?>
                                </span>
                            </div>
                            <div class="text-2xl font-extrabold text-brand mb-4">Rs
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div
                                class="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                                <span class="flex items-center gap-1"><i class="fas fa-map-marker-alt text-brand"></i>
                                ...
                                <?php if ($ad['category_name']): ?>
                                    <span class="flex items-center gap-1"><i class="fas fa-tag"></i>
        ...
        <?php else: ?>
            <div
                class="bg-white border border-slate-200 border-dashed rounded-2xl p-16 text-center text-slate-500 flex flex-col items-center justify-center">
                ...
                <a href="post-ad.php"
                    class="bg-brand text-white font-bold px-6 py-2.5 rounded-full hover:bg-brand-light transition flex items-center gap-2">
